...
import ge.ibsu.demo.dto.projections.CustomerAddress;
...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    ...
    @Query("select new ge.ibsu.demo.dto.projections.CustomerAddress(c.firstName, c.lastName, c.address.address, c.address.city.city, c.address.city.country.country) " +
            "From Customer c " +
            "where c.active = :active and " +
            "concat(c.firstName, concat(' ', c.lastName)) like :searchValue")
    Page<CustomerAddress> searchAddressInfo(@Param("searchValue") String searchValue, Pageable pageable);
